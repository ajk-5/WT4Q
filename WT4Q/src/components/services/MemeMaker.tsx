...
type Placement = "center" | "above" | "below";
...
export default function MemeMaker() {
  ...
  useEffect(() => {
    ...
    setStageW(newW);
    setStageH(newH);
  }, [aspectChoice, customW, customH, viewportW, currentRatio, stageW]);
  ...
  const onResizeMouseDown = (e: React.MouseEvent) => {
    e.preventDefault();
    const lockNow = lockAspect && !e.shiftKey;
    ...
    <div style={styles.page}>
      ...
      <div style={styles.controls} className="mm-controls">
        ...
        <div style={styles.row} className="mm-row">
          ...
          <button type="button" onClick={matchMediaAspect} style={styles.btn}>
            Match media aspect
          </button>
        </div>
        ...
        <div style={styles.row} className="mm-row">
          ...
          <button type="button" {...getRootProps()} style={styles.btn}>
            <input {...getInputProps()} />
...
const styles: Record<string, React.CSSProperties> = {
  ...
  label: { display: "flex", gap: 8, alignItems: "center", minWidth: 0 },
  select: {
    background: "#1a1440",
    color: "#fff",
